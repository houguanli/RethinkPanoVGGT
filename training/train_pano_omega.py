...
DEFAULT_DATASET_ROOT = PROJECT_ROOT / "data" / "mixed4"
DEFAULT_CHECKPOINT = PROJECT_ROOT / "checkpoints" / "vggt_omega_1b_512.pt"
CONFIG_PATH_KEYS = {
    "dataset_root",
    "checkpoint",
    "base_checkpoint",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--debug-dir", type=Path, default=Path("outputs/debug"))
...
def resolve_checkpoint_reference(reference: str | Path, owner_checkpoint: Path) -> Path:
    """Resolve checkpoint links after moving a run to another machine.

    Set ``P2P_VGGT_CHECKPOINT_OVERRIDE`` to an exact replacement path when a
    delta checkpoint refers to a foundation checkpoint from another system.
    Relative references are also checked from the current working directory,
    project root, and the directory containing the owner checkpoint.
    """

    original = Path(reference).expanduser()
    candidates: list[Path] = []
    override = os.environ.get("P2P_VGGT_CHECKPOINT_OVERRIDE", "").strip()
    if override:
