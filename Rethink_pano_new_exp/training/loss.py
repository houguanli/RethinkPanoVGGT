...
try:
    from .pano_loss import compute_pano_geometry_loss, has_pano_supervision
    from .luna_loss import compute_luna_consistency_loss, has_luna_supervision
except ImportError:
    from pano_loss import compute_pano_geometry_loss, has_pano_supervision
    from luna_loss import compute_luna_consistency_loss, has_luna_supervision
...
class MultitaskLoss(torch.nn.Module):
    ...
    """
    def __init__(self, camera=None, depth=None, point=None, track=None, pano=None, luna=None, **kwargs):
        super().__init__()
    ...
    def forward(self, predictions, batch) -> torch.Tensor:
        ...
        if self.pano is not None and has_pano_supervision(batch):
            pano_loss_dict = compute_pano_geometry_loss(predictions, batch, **self.pano)
            total_loss = total_loss + pano_loss_dict["loss_pano"] * self.pano.get("weight", 1.0)
            loss_dict.update(pano_loss_dict)

        if self.luna is not None and has_luna_supervision(batch):
            luna_loss_dict = compute_luna_consistency_loss(predictions, batch, **self.luna)
            total_loss = total_loss + luna_loss_dict["loss_luna"] * self.luna.get("weight", 1.0)
            loss_dict.update(luna_loss_dict)
