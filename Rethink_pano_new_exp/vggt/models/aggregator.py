...
class Aggregator(nn.Module):
    ...
    def __init__(
        ...
        if self.enable_pano_global_token:
            self.pano_global_token = nn.Parameter(torch.randn(1, 1, 1, embed_dim))
        ...
        self.luna_patch_layers = _resolve_luna_layers(luna_patch_layers, depth) if enable_luna else set()
        self.luna_camera_layers = _resolve_luna_layers(luna_camera_layers, depth) if enable_luna else set()
        ...
        self.patch_start_idx = 1 + num_register_tokens + int(self.enable_pano_global_token)

        # Initialize parameters with small values
        nn.init.normal_(self.camera_token, std=1e-6)
        nn.init.normal_(self.register_token, std=1e-6)
        if self.pano_global_token is not None:
            nn.init.normal_(self.pano_global_token, std=1e-6)
    ...
    def forward(
        self,
        images: torch.Tensor,
        pano_geometry: torch.Tensor = None,
        pano_token_meta: Dict[str, torch.Tensor] = None,
        pano_camera_meta: torch.Tensor = None,
        ...
        if self.enable_pano_global_token:
            pano_global_token = expand_and_flatten(self.pano_global_token, B, S)
            if pano_geometry is not None:
                if pano_geometry.shape[:2] != (B, S):
                    raise ValueError(
                        f"Expected pano_geometry shape [B, S, D] with B={B}, S={S}, got {pano_geometry.shape}"
                    )
                geometry_delta = self.pano_geometry_embed(pano_geometry.to(device=images.device, dtype=patch_tokens.dtype))
                geometry_delta = geometry_delta.reshape(B * S, 1, C)
        ...
        for _ in range(self.aa_block_num):
            ...
            luna_layer_idx = global_idx - 1
            if self.enable_luna:
                tokens, luna_tokens_bsp = self._process_luna_adapters(
                    tokens=tokens,
                    B=B,
                    S=S,
                    P=P,
                    C=C,
                    layer_idx=luna_layer_idx,
                    pano_token_meta=pano_token_meta,
                    pano_camera_meta=pano_camera_meta,
                )
                if luna_tokens_bsp is not None:
                    global_intermediates[-1] = luna_tokens_bsp
    ...
    def _process_luna_adapters(
        ...
        """Apply optional LUNA residual adapters after a global block."""
        ...
        tokens_bsp = tokens.view(B, S, P, C)
        if has_patch:
            special_tokens = tokens_bsp[:, :, : self.patch_start_idx, :]
            patch_tokens = tokens_bsp[:, :, self.patch_start_idx :, :]
        ...
        if has_camera:
            camera_token = self.luna_camera_adapters[layer_key](tokens_bsp[:, :, 0, :], pano_camera_meta)
            tokens_bsp = torch.cat([camera_token[:, :, None, :], tokens_bsp[:, :, 1:, :]], dim=2)

        return tokens_bsp.reshape(B, S * P, C), tokens_bsp
...
def expand_and_flatten(token_tensor, B, S):
    """Expands a single shared token to every view in the panorama sequence."""
    expanded = token_tensor.expand(B, S, *token_tensor.shape[2:])
    return expanded.reshape(B * S, *expanded.shape[2:])


def _resolve_luna_layers(layers, depth):
    if layers is None:
