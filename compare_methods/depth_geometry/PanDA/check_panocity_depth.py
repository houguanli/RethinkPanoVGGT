...
import networks.panda  # noqa: F401,E402 - registers the PanDA model.
...
def build_dataset(config: Dict, split: str, max_samples: int | None) -> PanoCity:
    ...
    return PanoCity(
        ...
        height=args.get("height", 518),
        width=args.get("width", 1036),
        ...
        max_depth_meters=args.get("max_depth_meters", 10.0),
        target_mode=args.get("target_mode", "metric"),
    )


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="config/metric_depth/train_panocity_4rtx5000.yaml")
    ...
    max_depth = float(getattr(dataset, "max_depth_meters", 10.0))
    summary = {
        "method": "panda",
        "split": args.split,
        "samples": len(dataset),
    ...
    if not args.no_forward:
        ...
        pred = outputs["pred_depth"].detach().cpu()
