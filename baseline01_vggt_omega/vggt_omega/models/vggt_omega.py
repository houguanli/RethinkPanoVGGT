    ...
    def forward(self, images: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        with torch.autocast(device_type="cuda", enabled=False):
            if self.camera_head is not None:
                ...
                # Baseline01's training losses expect the original VGGT
                # multi-stage key. Omega exposes a single camera stage, so we
                # wrap it in a one-element list for compatibility.
