...
CONFIG="${CONFIG:-mixed4_pano_vggtomega_currentloss_ablation_4090_12h}"
...
EVAL_OUT="${EVAL_OUT:-/mnt/e/PanoVGGT_minimal_datasets/eval_outputs/${CONFIG}}"
...
if [[ -z "$PRED_DEPTH_SCALE" ]]; then
  echo "[ablation] PRED_DEPTH_SCALE is empty; run calibration or set PRED_DEPTH_SCALE explicitly." | tee -a "$SEQ_LOG"
  exit 1
fi
echo "[ablation] unified pred_depth_scale=$PRED_DEPTH_SCALE" | tee -a "$SEQ_LOG"

echo "[ablation] training pure VGGTOmega current-loss baseline started $(date --iso-8601=seconds)" | tee -a "$SEQ_LOG"
cd "$BASELINE"
...
PYTHONPATH="$BASELINE${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON" scripts/evaluate_mixed4_depth_checkpoint.py \
  --config "$CONFIG" \
  --checkpoint "$CKPT" \
  ...
  --seed 123 \
  2>&1 | tee "$EVAL_OUT/validation_mixed4_by_dataset_valtestfull_console.log"
