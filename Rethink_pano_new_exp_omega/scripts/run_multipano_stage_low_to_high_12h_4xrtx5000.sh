...
LUNA_CONFIG="configs/multipano_rtx5000x4_mixed4_pano_low_to_high_luna_after_full_warmup_9h.yaml"
LUNA_OUT="$LUNA/logs/mixed4_pano_low_to_high_4xrtx5000_multipano_after_full_warmup_9h"
SEQ_LOG="$LUNA/logs/mixed4_pano_low_to_high_4xrtx5000_multipano_stage_12h_sequence.log"

mkdir -p "$(dirname "$SEQ_LOG")"
if [[ "${CLEAN_OUTPUT:-0}" == "1" ]]; then
  rm -rf "$BASE_OUT" "$LUNA_OUT" "$SEQ_LOG" "$LUNA/logs/debug_mixed4_pano_low_to_high_4xrtx5000_multipano_after_full_warmup_9h"
fi
...
if [[ -s "$BASE_OUT/loss.csv" && -s "$LUNA_OUT/loss.csv" ]]; then
  "$PYTHON" scripts/plot_loss_csv.py \
    ...
    --title "Mixed4 low-to-high 3h warmup + 9h multi-pano LUNA robust smoothed loss" \
    2>&1 | tee -a "$SEQ_LOG"
fi
