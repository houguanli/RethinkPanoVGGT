...
LUNA_OUT="${LUNA_OUT:-$LUNA/logs/mixed4_pano_fixed8window_4xrtx5000_multipano_after_full_warmup_9h}"
SEQ_LOG="${SEQ_LOG:-$LUNA/logs/mixed4_pano_fixed8window_4xrtx5000_multipano_stage_12h_sequence.log}"
...
if [[ "${CLEAN_OUTPUT:-0}" == "1" ]]; then
  rm -rf "$WARMUP_OUT" "$LUNA_OUT" "$SEQ_LOG" "$LUNA/logs/debug_mixed4_pano_fixed8window_4xrtx5000_multipano_after_full_warmup_9h"
fi
