...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--luna-patch-bank-mode",
        choices=["aligned", "none", "shuffled"],
        default="aligned",
        help=(
            "Patch Bank condition: aligned is the full method, none is a "
            "parameter-matched zero-context control, and shuffled breaks ERP correspondence."
        ),
    )
    parser.add_argument(
        "--luna-patch-bank-shuffle-seed",
        type=int,
        default=None,
        help="Optional deterministic seed for shuffled Patch Bank lookup.",
    )
    parser.add_argument(
        "--disable-geora",
        action="store_true",
        default=False,
        help="Build the split-window model without Patch or Camera GeoRA adapters.",
    )
...
def _load_config_mapping(path: Path, stack: tuple[Path, ...] = ()) -> Dict:
    """Load a YAML config with optional recursive ``base_config`` inheritance."""
    ...
    base = _load_config_mapping(base_path, (*stack, resolved))
...
def train(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        rank0_print(
            f"[INFO] geora_enabled = {not args.disable_geora}; "
            f"patch_bank_mode = {args.luna_patch_bank_mode}; "
            f"patch_bank_shuffle_seed = {args.luna_patch_bank_shuffle_seed}",
            dist_state,
        )
        ...
        if args.training_stages:
            ...
            rank0_print(
                f"[INFO] active_stage = {format_stage_status(active_stage_index, active_stage, trainable_count, frozen_count, disable_geora=args.disable_geora)}",
                dist_state,
            )
...
def format_stage_status(
    ...
    trainable_count: int,
    frozen_count: int,
    disable_geora: bool = False,
) -> str:
    if stage is None:
        return f"default trainable={trainable_count:,} frozen={frozen_count:,}"
    effective_luna_forward = not disable_geora and bool(stage.get("enable_luna_forward", True))
    return (
        ...
        f"luna_forward={effective_luna_forward} "
...
def configure_trainable_for_stage(
    ...
    enable_luna_forward = not args.disable_geora and bool(stage.get("enable_luna_forward", True))
    set_luna_forward_enabled(base_model, enable_luna_forward)
    if not args.disable_geora and (
        not enable_luna_forward or bool(stage.get("freeze_luna_parameters", False))
    ):
...
def build_model(args: argparse.Namespace) -> VGGTOmega_LUNA:
    ...
    if args.smoke:
        ...
        model = VGGTOmega_LUNA(
            ...
            enable_luna=not args.disable_geora,
            luna_patch_layers=args.luna_patch_layers,
            luna_camera_layers=args.luna_camera_layers,
            luna_patch_bank_mode=args.luna_patch_bank_mode,
            luna_patch_bank_shuffle_seed=args.luna_patch_bank_shuffle_seed,
